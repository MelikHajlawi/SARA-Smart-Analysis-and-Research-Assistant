...
- Redis Caching with Dynamic TTLs 
...
- Implements robust error handling and logging (debateable !!)
...
from fastapi import (
    FastAPI, HTTPException, Depends, Security, Request, Body, status
)
...
def generate_cache_key(request: ProcessingRequest, bucket_start: datetime, bucket_end: datetime) -> bytes:
    ...
    """
    agent= ",".join(request.agent_serial)  
    key_str = (
...
# was not used, front doesnt include event_count, activity_percent, event_duration_avg, event_duration_max as metrics, i should remove it !!!
...
# can add more metrics here for statistical analysis
...
# --------------------------
# Main API Endpoint: Process Data 
...
async def process_data_batched_stream(
    ...
    """
    # 1) Call the original handler to get the full response dict
